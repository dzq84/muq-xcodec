...
class CODEC(L.LightningModule):
    def __init__(
        self,
        encoder_dim: int = 64,
        encoder_rates: List[int] = [2, 4, 10, 12],
        latent_dim: int = None,
        decoder_dim: int = 1536,
        decoder_rates: List[int] = [12, 10, 4, 2],
        n_codebooks: int = 8,
        ...
        self.semantic = MuQ.from_pretrained("OpenMuQ/MuQ-large-msd-iter").eval()

        self.n_codebooks = n_codebooks
        self.codebook_size = codebook_size
        self.codebook_dim = codebook_dim
        self.fc_prior = nn.Linear(latent_dim*2,latent_dim*2)     
        self.fc_post1 = nn.Linear(latent_dim*2,latent_dim)     
        self.fc_post2 = nn.Linear(latent_dim*2,latent_dim)  
    ...
    def training_step(self, batch, batch_idx):
        ...
        z_a = self.encoder(audio_data).permute(0,2,1)
        with torch.no_grad():
            z_s = self.semantic(audio_data, output_hidden_states=True).last_hidden_state #MuQ
        
        z = torch.cat((z_a, z_s), dim=2)
        z = self.fc_prior(z).permute(0,2,1)
        
        z_q, codes, latents, commitment_loss, codebook_loss = self.quantizer(z)
        z_q = z_q.permute(0,2,1)
        z_s_q = self.fc_post1(z_q)
        z_a_q = self.fc_post2(z_q).permute(0,2,1)
        
        audio_data_hat = self.decoder(z_a_q)
        ...
        s_loss = closs.MseLoss(z_s,z_s_q)
        loss_g = 0.25 * commitment_loss + codebook_loss + 15 * mel_loss + g_loss + 2 * f_loss + s_loss *10
