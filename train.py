    ...
    def __init__(self, root_dir, sample_rate=16000, segment_length=1):
        ...
        for dirpath, _, filenames in os.walk(root_dir):
            for filename in filenames:
                if filename.endswith('.wav') or filename.endswith('.mp3') or filename.endswith('.opus'):
...
if __name__ == "__main__":
    ...
    trainer = Trainer(accelerator='gpu', devices=config['devices'], strategy=DDPStrategy(find_unused_parameters=True),logger=logger,callbacks=[checkpoint_callback])
